...
var DatabaseManager = (function () {
  ...
  DatabaseManager.prototype.param = function (i) {
    return '?';
  };
  DatabaseManager.prototype.exec = function (sql, args, ctx) {
    var p = (ctx ? ctx : this.database);
    return exec(p, sql, args);
  };
  DatabaseManager.prototype.execBatch = function (statements, firstSuccess, ctx) {
    var p = (ctx ? ctx : this.database);
    return execBatch(p, statements, firstSuccess);
  };
  DatabaseManager.prototype.query = function (sql, args, m, fields, ctx) {
    var p = (ctx ? ctx : this.database);
    return query(p, sql, args, m, fields);
  };
  DatabaseManager.prototype.queryOne = function (sql, args, m, fields, ctx) {
    var p = (ctx ? ctx : this.database);
    return queryOne(p, sql, args, m, fields);
  };
  DatabaseManager.prototype.execScalar = function (sql, args, ctx) {
    var p = (ctx ? ctx : this.database);
    return execScalar(p, sql, args);
  };
  DatabaseManager.prototype.count = function (sql, args, ctx) {
    var p = (ctx ? ctx : this.database);
...
function execBatch(db, statements, firstSuccess) {
  ...
  if (firstSuccess) {
    return new Promise(function (resolve, reject) {
      return db.transaction(function (txn) {
        return txn.executeSql(statements[0].query, statements[0].params, function (tx, result) {
          if (result && result.rowsAffected > 0) {
            var subs = statements.slice(1);
...
function exec(db, sql, args) {
  ...
  if (db.executeSql) {
    return new Promise(function (resolve, reject) {
      db.executeSql(sql, p, function (tx, result) {
        if (result && result.rowsAffected > 0) {
          return resolve(1);
        }
        else if (!result) {
...
function query(db, sql, args, m, bools) {
  ...
  if (db.executeSql) {
    return new Promise(function (resolve, reject) {
      db.executeSql(sql, p, function (_tx, results) {
...
function queryOne(db, sql, args, m, bools) {
  ...
  if (db.executeSql) {
    return new Promise(function (resolve, reject) {
      db.executeSql(sql, p, function (_tx, result) {
